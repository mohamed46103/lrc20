...
use lrcdk::AnyBitcoinProvider;
use lrcdk::bitcoin_provider::{BitcoinProvider, BitcoinProviderConfig};
use lrcdk::wallet::{StorageWallet, StorageWalletConfig, SyncOptions, WalletConfig};
...
impl Context {
    ...
    pub fn lrc20_client(&mut self) -> eyre::Result<HttpClient> {
        let cfg = self.config()?;

        let client = HttpClientBuilder::new().build(cfg.lrc20_rpc.url.clone())?;

        self.lrc20_client = Some(client.clone());
    ...
    pub async fn wallet(&mut self) -> eyre::Result<Arc<StorageWallet>> {
        if let Some(wallet) = &self.lrc20_wallet {
        ...
        let wallet = StorageWallet::from_storage_config(StorageWalletConfig {
            inner: WalletConfig {
                ...
                lrc20_url: config.lrc20_rpc.url.clone(),
        ...
        let pb = setup_progress_bar("Syncing lrc20 and bitcoin wallets...".into());
        wallet.sync(SyncOptions::default()).await?;
        pb.finish();

        let wallet = Arc::new(wallet);
        self.lrc20_wallet = Some(wallet.clone());
