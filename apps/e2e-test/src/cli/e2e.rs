...
impl E2e {
    ...
    pub async fn run(&self) -> eyre::Result<()> {
        info!(
            "Starting the end-to-end test with {} account(s), {} LRC20 node(s) and {} Bitcoin node(s)",
            self.config.accounts.number,
            self.config.nodes.lrc20.len(),
    ...
    /// Generate a keypair with random LRC20 and Bitcoin nodes.
    ...
    async fn generate_account(
        ...
        // Pick a random LRC20 node for the account.
        let (lrc20_url, lrc20_client) = config.get_lrc20_node()?;
        ...
        let wallet =
            Self::setup_wallet_from_provider(private_key, lrc20_url, provider_config).await?;
