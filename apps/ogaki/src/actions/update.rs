...
use crate::utils::{get_current_version, try_get_lrc20d_path};

/// Configuration for the ogaki commands.
#[derive(Args, Debug, Clone)]
pub struct UpdateArgs {
    /// Github repository URL to pool for updates, defaults to the official lrc20d repository.
    #[clap(short, long)]
...
/// Unpack update tar.gz archive from a byte slice to the lrc20d binary directory.
fn unpack_tar_gz_from_bytes(bytes: &[u8]) -> Result<(), Error> {
    let destination = try_get_lrc20d_path()?
        .parent()
        .wrap_err("LRC20d binary path has no parent directory")?
        .to_path_buf();
