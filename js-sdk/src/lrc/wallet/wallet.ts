...
import { BitcoinTransactionDto, BtcMetadata } from "../types/bitcoin-transaction";
import { BitcoinUtxo, Lrc20Utxo } from "../types/bitcoin-utxo";
import { TokenPubkey, TokenPubkeyInfo } from "../types/token-pubkey";
import { BitcoinInput, MultisigReceiptInput, ReceiptInput, TxInput } from "../types/input";
import { TokenAmount } from "../types/token-amount";
import { BitcoinOutput, MultisigReceiptOutput, ReceiptOutput, SparkExitOutput, TxOutput } from "../types/output";
import { Payment } from "../types/payment";
import { Receipt } from "../types/receipt";
import { getReceiptDataFromProof, ReceiptProof, ReceiptProofType, SigReceiptProofData } from "../types/receipt-proof";
import {
  AnnouncementData,
...
import { reverseBuffer, toEvenParity, toXOnly } from "../utils/buffer";
import { DUST_AMOUNT, EMPTY_TOKEN_PUBKEY, ESPLORA_URL, ESPLORA_AUTH, LRC_NODE_URL } from "../utils/constants";
...
export class LRCWallet {
  ...
  private lastLrc20Page: number = 0;
  ...
  protected init() {
    ...
    this.esploraApi = new EsploraApi(
      ESPLORA_URL[this.networkType] || ESPLORA_URL["default"],
      ESPLORA_AUTH[this.networkType] || null
    );
  ...
  private async fetchUtxos(address: string): Promise<BitcoinUtxo[]> {
    ...
    let currentHeight = await this.esploraApi.getLastBlockHeight();

    let inputs = [];
    let outputs = new Map<{ txid: string; vout: number }, EsploraTransactionOutput>();
    let txStatuses = {};
    txs.forEach((tx) => {
      // The output of a coinbase transaction can only be spent after the transaction
      // is 100 blocks deep in the blockchain.
      const firstVin = tx.vin[0];
      if (firstVin.is_coinbase && tx.status.block_height - 1 + COINBASE_MATURITY > currentHeight) {
        return;
      }
  ...
  private createOutputProofs(outputs: TxOutput[], index: number = 0): Map<number, ReceiptProof> {
  ...
  public async coinselect(
    tokenPubkey: string,
    amount: bigint,
    feeRate: number = 1.0
  ): Promise<Array<BitcoinUtxo | Lrc20Utxo>> {
