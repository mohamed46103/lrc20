...
use eyre::bail;
use lrc20_types::{ControllerMessage, TxConfirmMessage};
    ...
    BC: BitcoinRpcApi + Send + Sync + 'static,
{
    ...
    async fn handle_new_block(&mut self, block: GetBlockTxResult) -> eyre::Result<()> {
        ...
        if self.latest_blocks.len() == self.confirmations_number as usize {
            ...
            let lrc20_txs = self.extract_waiting_txs_from_block(&confirmed_block);
            if !lrc20_txs.is_empty() {
                self.new_confirmed_txs(&lrc20_txs).await;
    ...
    async fn new_confirmed_txs(&mut self, lrc20_tx_ids: &[Txid]) {
        tracing::debug!("Transactions confirmed: {:?}", lrc20_tx_ids);
        for tx_id in lrc20_tx_ids {
            self.queue.remove(tx_id);
        }

        self.event_bus
            .send(ControllerMessage::ConfirmedTxs(lrc20_tx_ids.to_vec()))
            .await;
    }
}
