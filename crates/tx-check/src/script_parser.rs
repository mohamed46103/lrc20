...
impl SpendingCondition {
    ...
    /// Derive the spending condition from a P2TR scriptPubKey.
    pub(crate) fn p2tr(prev_out_script: &ScriptBuf) -> Result<Self, ScriptParseError> {
        if !prev_out_script.is_p2tr() {
            return Err(ScriptParseError::UnsupportedScript);
        }

        Ok(Self::P2TR(TaprootData(prev_out_script.clone())))
    }
    ...
    pub fn into_script(self) -> ScriptBuf {
        match self {
            ...
            | SpendingCondition::P2WSH(ScriptHashData(script))
            | SpendingCondition::P2TR(TaprootData(script)) => script,
        }
    }
}
...
impl PubkeyHashData {
    ...
    pub fn from_witness(witness: &Witness) -> Self {
        ...
        let script_pub_key = ScriptBuf::new_p2wpkh(&WPubkeyHash::from_raw_hash(pubkey_hash));
...
impl ScriptHashData {
    ...
    pub fn from_witness(witness: &Witness) -> Result<Self, ScriptParseError> {
        ...
        let script_pub_key = ScriptBuf::new_p2wsh(&WScriptHash::from_raw_hash(script_hash));
...
/// Used to represent `P2TR` scriptPubKey.
#[derive(Debug, Clone)]
pub struct TaprootData(pub ScriptBuf);
