...
use bitcoin::{
    ...
    key::Secp256k1,
    opcodes::{OP_0, all::OP_PUSHBYTES_71},
    script::Builder,
};
...
fn create_p2wpkh_script_pub_key(pubkey: &PublicKey) -> ScriptBuf {
    ...
    ScriptBuf::new_p2wpkh(&WPubkeyHash::from_raw_hash(pubkey_hash))
}
...
fn create_p2wsh_script_pub_key(pubkey: &PublicKey) -> ScriptBuf {
    ...
    ScriptBuf::new_p2wsh(&WScriptHash::from_raw_hash(script_hash))
}

fn create_p2tr_script_pub_key(pubkey: &PublicKey) -> ScriptBuf {
    ScriptBuf::new_p2tr(&Secp256k1::new(), pubkey.inner.x_only_public_key().0, None)
}
...
#[test]
fn test_from_script_pubkey_p2tr() {
    let script_pub_key = create_p2tr_script_pub_key(&PUBKEY);

    let spending_condition =
        SpendingCondition::p2tr(&script_pub_key).expect("Failed to derive P2TR scriptPubKey");
...
fn parse_invalid_scripts() {
    ...
    // Test P2TR spending condition with wrong scriptPubKey.
    let script_pub_key = create_p2wsh_script_pub_key(&PUBKEY);
    match SpendingCondition::p2tr(&script_pub_key) {
        Err(err) => {
            assert_eq!(err, ScriptParseError::UnsupportedScript);
        }
        err => {
            panic!("Unexpected result: {:?}", err);
        }
