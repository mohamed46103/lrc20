...
use bitcoin::{WScriptHash, blockdata::script::Instruction, hashes::Hash};

use crate::{CheckableProof, Receipt, Tweakable};
...
pub struct LightningHtlcProof {
    /// Receipt that is locked by HTLC.
    pub receipt: Receipt,
...
impl LightningHtlcProof {
    pub fn new(receipt: Receipt, data: LightningHtlcData) -> Self {
        Self { receipt, data }
    }
    ...
    fn to_script(&self) -> LightningHtlcScript {
        let remote_key = self.data.remote_htlc_key.tweak(self.receipt);
