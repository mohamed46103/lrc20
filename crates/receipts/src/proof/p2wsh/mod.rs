...
pub struct P2WSHProof {
    /// The receipt that is being spent
    pub receipt: Receipt,
...
impl P2WSHProof {
    pub fn new(receipt: Receipt, pubkey: PublicKey, script: ScriptBuf) -> Self {
        Self {
            receipt,
            inner_key: pubkey,
            script,
    ...
    fn check_pubkey(&self) -> Result<(), P2WSHProofError> {
        let proof_tweaked_pubkey = self.inner_key.tweak(self.receipt);
