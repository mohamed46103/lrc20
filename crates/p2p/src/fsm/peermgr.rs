...
lazy_static::lazy_static! {
    static ref PEERS: IntGaugeVec = register_int_gauge_vec!(
        "lrc20_peers",
...
enum HandshakeState {
    /// Received "version" and waiting for "ltxidack" message from remote.
    Version { since: LocalTime },
    /// Received "ltxidack" and waiting for "verack" message from remote
    Ltxidack { since: LocalTime },
...
#[derive(Debug, Clone)]
pub struct Peer {
    /// The [`PeerState`] of the peer, indicating connection status and other information.
    pub state: PeerState,

    /// The [`PeerAddr`] corresponding to the peer, if the peer is a persistent peer.
...
pub struct PeerInfo {
    ...
    pub relay: bool,
    /// The max protocol version supported by both the peer and lrc20_p2p.
    pub version: u32,
    /// Whether this is a persistent peer.
    pub persistent: bool,
    /// Whether this peer supports BIP-339.
    pub wtxidrelay: bool,
    /// Whether this peer use LRC20 protocol
    pub ltxidrelay: bool,
...
pub struct PeerManager<U, C> {
    ...
    persistent_disconnected: HashMap<PeerAddr, (Option<LocalTime>, usize)>,
...
impl<U: Wire<Event> + SetTimer + Connect + Disconnect, C: Clock + Sync> PeerManager<U, C> {
    ...
    pub fn new(
        ...
        Self {
            ...
            last_idle: None,
            peers_storage,
            persistent_disconnected: disconnected,
            upstream,
            rng,
            clock,
            network,
    ...
    pub async fn initialize<A: AddressSource>(&mut self, addrs: &mut A) {
        ...
        for peer in peers {
            let addrs: Result<vec::IntoIter<SocketAddr>, Error> = peer.clone().try_into();

            match addrs {
                Ok(addrs) => {
                    for addr in addrs {
                        if !self.connect(&addr, Some(peer.clone())) {
                            debug!("Unable to connect to persistent peer {peer}");
                        } else {
                            debug!("Connected to persistent peer {peer} at address {addr}")
                        }
                    }
                }
                Err(err) => {
                    error!("Failed to resolve persistent peer address {peer}: {err}");
                    let (retry_at, attempts) =
                        self.persistent_disconnected.entry(peer).or_default();

                    *retry_at = Some(local_time + LocalDuration::from_secs(10));
                    *attempts = 1
                }
            }
        }

        if self.persistent_disconnected.is_empty() {
            self.upstream.set_timer(IDLE_TIMEOUT);
        } else {
            // If we have any persistent peers that we couldn't connect to, set a shorter timer so
            // we get back to trying to establish a connection.
            self.upstream.set_timer(LocalDuration::from_secs(10));
        }
    ...
    fn persistent_disconnected(&mut self, addr: PeerAddr, local_time: LocalTime) {
        let (retry_at, attempts) = self
            .persistent_disconnected
            .entry(addr.clone())
            .or_default();
        ...
        *attempts += 1;
        ...
        debug!("Persistent peer {addr} disconnected, retrying in {delay:?}");
    ...
    fn maintain_persistent(&mut self) {
        ...
        for (peer_addr, (retry_at, _)) in &mut self.persistent_disconnected {
        ...
        debug!("Maintaining persistent peers: {peer_addrs:?}");

        for peer_addr in peer_addrs {
            let addrs: Result<vec::IntoIter<SocketAddr>, Error> = peer_addr.clone().try_into();

            match addrs {
                Ok(addrs) => {
                    for addr in addrs {
                        if !self.connect(&addr, Some(peer_addr.clone())) {
                            error!(target: "p2p", "Couldn't establish connection with {addr}");
                        } else {
                            debug!(target: "p2p", "Connected to persistent peer {peer_addr} at address {addr}");
                        }
                    }
                }
                Err(err) => {
                    error!(target: "p2p", "Failed to resolve persistent peer address {peer_addr}: {err}");
                }
            }
        }
    }
    ...
    pub fn peer_connected(&mut self, addr: SocketAddr, local_addr: SocketAddr, link: Link) -> bool {
        ...
        let prev_peer = self.peers_storage.insert(
            addr,
            Peer {
        ...
        if let Some(addr) = prev_peer.and_then(|peer| peer.addr) {
            self.persistent_disconnected.remove(&addr);
        }
    ...
    pub async fn peer_disconnected<A: AddressSource>(
        ...
        if let Some(peer_addr) = self.peers_storage.remove(addr).and_then(|peer| peer.addr) {
            // This is a persistent peer that has been disconnected, make sure to add it to our
            // disconnected map so that we reconnect to it later.
            self.persistent_disconnected(peer_addr, local_time);
        } else {
            self.maintain_connections(addrs).await;
        }
    ...
    pub fn received_wtxidrelay(&mut self, addr: &PeerId) {
        if let Some(PeerState::Connected {
            peer: Some(peer),
            conn: _,
        }) = self.peers_storage.get_mut(addr).map(|peer| &mut peer.state)
        {
            match peer.state {
    ...
    /// Called when a `ltxidrelay` message was received.
    pub fn received_ltxidrelay(&mut self, addr: &PeerId) {
        if let Some(PeerState::Connected {
            peer: Some(peer),
            conn: _,
        }) = self.peers_storage.get_mut(addr).map(|peer| &mut peer.state)
        {
            match peer.state {
                HandshakeState::Version { .. } => {
                    peer.ltxidrelay = true;
                    self.upstream.ltxidack(*addr);
                    self.upstream.verack(*addr);
                }
                _ => self.disconnect(
                    *addr,
                    DisconnectReason::PeerMisbehaving(
                        "`ltxidrelay` must be received before `verack`",
                    ),
                ),
            }
        }
    ...
    fn handle_version<A: AddressSource>(
        ...
        if let Some(peer) = self.peers_storage.get(addr) {
            if let PeerState::Connected { ref conn, .. } = peer.state {
                ...
                let persistent = peer
                    .addr
                    .as_ref()
                    .is_some_and(|addr| self.config.persistent.contains(addr));
                    ...
                    Peer {
                        state: PeerState::Connected {
                            ...
                            peer: Some(PeerInfo {
                                ...
                                wtxidrelay: false,
                                ltxidrelay: true, // for now we assume that every node supports LRC20 protocol by default
                            }),
                        },
    ...
    pub fn received_ltxidack(&mut self, addr: &PeerId, local_time: LocalTime) {
        if let Some(PeerState::Connected {
            peer: Some(peer),
            conn: _,
        }) = self.peers_storage.get_mut(addr).map(|peer| &mut peer.state)
        {
            if let HandshakeState::Version { .. } = peer.state {
                peer.state = HandshakeState::Ltxidack { since: local_time }
            } else {
                self._disconnect(
                    *addr,
                    DisconnectReason::PeerMisbehaving("unexpected `ltxidack` message received"),
                );
            }
        }
    }
    ...
    pub fn received_verack(
        ...
        {
            if let HandshakeState::Ltxidack { .. } = peer.state {
    ...
    pub async fn received_wake<A: AddressSource>(&mut self, addrs: &mut A) {
        ...
        for (peer, conn) in self.peers() {
            match peer.state {
                ...
                HandshakeState::Ltxidack { .. } | HandshakeState::Verack { .. } => {}
            }
        }
        // Time out peers that haven't sent a `version` quickly enough.
        for connected in self.peers_storage.values().filter_map(|c| match c.state {
            PeerState::Connected {
                ref conn,
                peer: None,
            } => Some(conn),
    ...
    fn init_metrics(&self) {
        PEERS.with_label_values(&["connecting"]).set(0);
        PEERS.with_label_values(&["connected"]).set(0);
    }

    fn sync_metrics(&self) {
        let (mut connecting, mut connected) = (0, 0);

        for peer in self.peers_storage.values() {
            match peer.state {
        ...
        PEERS.with_label_values(&["connecting"]).set(connecting);
        PEERS.with_label_values(&["connected"]).set(connected);
    }
}
...
impl<U: Connect + Disconnect + SetTimer + Wire<Event>, C: Clock + Sync> PeerManager<U, C> {
    ...
    pub fn is_inbound(&mut self, addr: &PeerId) -> bool {
        self.peers_storage.get(addr).is_some_and(
            |peer| matches!(peer.state, PeerState::Connected { ref conn, .. } if conn.link.is_inbound()),
        )
    }
    ...
    pub fn is_connecting(&self, addr: &PeerId) -> bool {
        self.peers_storage
            .get(addr)
            .is_some_and(|peer| matches!(peer.state, PeerState::Connecting { .. }))
    }
    ...
    pub fn is_connected(&self, addr: &PeerId) -> bool {
        self.peers_storage
            .get(addr)
            .is_some_and(|peer| matches!(peer.state, PeerState::Connected { .. }))
    }
    ...
    pub fn peers(&self) -> impl Iterator<Item = (&PeerInfo, &Connection)> + Clone {
        self.peers_storage
            .values()
            .filter_map(move |c| match c.state {
                PeerState::Connected {
                    ref conn,
                    peer: Some(ref peer),
                } => Some((peer, conn)),
    ...
    pub fn connecting(&self) -> impl Iterator<Item = &PeerId> {
        self.peers_storage
            .iter()
            .filter(|(_, p)| matches!(p.state, PeerState::Connecting { .. }))
            .map(|(addr, _)| addr)
    }

    /// Iterator over peers in a *connected* state..
    pub fn connected(&self) -> impl Iterator<Item = &Connection> + Clone {
        self.peers_storage.values().filter_map(|c| match c.state {
    ...
    /// Connect to a peer.
    pub fn connect(&mut self, addr: &PeerId, peer_addr: Option<PeerAddr>) -> bool {
        ...
        self.peers_storage.insert(
            *addr,
            Peer {
                state: PeerState::Connecting { time },
                addr: peer_addr,
            },
        );
        ...
        while connecting.len() < delta {
            if let Some((addr, source)) = if negotiated < target {
                ...
                if self.get_dns_seed().is_empty() {
                    debug!(
                        "Tried to get more addresses from DNS seeds, however, there`s no DNS seeds provided"
                    );
                    break;
                }
    ...
    fn idle_peers(&self, now: LocalTime) -> impl Iterator<Item = PeerId> + '_ {
        self.peers_storage.iter().filter_map(move |(addr, c)| {
            if let PeerState::Connecting { time } = c.state {
