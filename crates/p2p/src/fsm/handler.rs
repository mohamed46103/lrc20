...
use lrc20_types::messages::p2p::{Inventory, NetworkMessage, RawNetworkMessage};
use lrc20_types::network::Network;
use lrc20_types::spark::TokenTransaction;
use lrc20_types::spark::signature::SparkSignatureData;
use lrc20_types::{ControllerMessage, ControllerP2PMessage, Lrc20Transaction};
...
pub const USER_AGENT: &str = concat!("/lrc20:", env!("CARGO_PKG_VERSION"), "/");

lazy_static::lazy_static! {
    static ref GOSSIP_MESSAGE_RECEIVED_TOTAL: IntCounterVec = register_int_counter_vec!(
        "lrc20_gossip_message_received_total",
        "Total number of received gossip messages",
        &["message"]
    ).unwrap();
}
...
pub enum Command {
    ...
    /// Send GetData message to the desired peer
    SendLrc20Transactions(Vec<Lrc20Transaction>, SocketAddr),
    /// Send GetData message to the desired peer
    SendSparkTransactions(Vec<TokenTransaction>, SocketAddr),
    /// Send GetData message to the desired peer
    SendSparkSignatures(Vec<SparkSignatureData>, SocketAddr),
...
impl fmt::Debug for Command {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::SendLrc20Transactions(msg, addr) => {
                write!(f, "SendLrc20Transactions({:?}) to {:?}", msg, addr)
            }
            Self::SendSparkTransactions(msg, addr) => {
                write!(f, "SendSparkTransactions({:?}) to {:?}", msg, addr)
            }
            Self::SendSparkSignatures(msg, addr) => {
                write!(f, "SendSparkSignatures({:?}) to {:?}", msg, addr)
            }
...
impl<P: peer::Store + Send, C: AdjustedClock<PeerId> + Sync + Send> StateMachine<P, C> {
    ...
    pub async fn command(&mut self, cmd: Command) {
        if let Command::SendInv { .. } = cmd {
            trace!(target: "p2p", "Received command: {:?}", cmd);
        } else {
            debug!(target: "p2p", "Received command: {:?}", cmd);
        }

        match cmd {
            // The start command is handled before this method so it's unnecessary to handle it
            // here.
            Command::Start => {}
            ...
            Command::Connect(addr) => {
                ...
                self.peermgr.connect(&addr, None);
            }
            ...
            Command::SendLrc20Transactions(txs, addr) => {
                self.send(NetworkMessage::Lrc20Tx(txs), addr);
            }
            Command::SendSparkTransactions(txs, addr) => {
                self.send(NetworkMessage::SparkTx(txs), addr);
            }
            Command::SendSparkSignatures(sigs, addr) => {
                self.send(NetworkMessage::SparkSig(sigs), addr);
            }
    ...
    for StateMachine<P, C>
{
    ...
    async fn message_received(&mut self, addr: &SocketAddr, msg: Cow<'_, RawNetworkMessage>) {
        ...
        if !self.peermgr.is_connected(&addr) {
            if let NetworkMessage::Inv { .. } = msg.payload {
                trace!(target: "p2p", "Received {:?} from unknown peer {}", msg, addr);
            } else {
        ...
        match msg.payload.clone() {
            ...
            NetworkMessage::Lrc20Tx(txs) => {
                self.event_bus
                    .send(ControllerMessage::P2P(ControllerP2PMessage::Lrc20Tx {
                        txs,
                        sender: addr,
                    }))
                    .await;
            }
            NetworkMessage::SparkTx(txs) => {
                self.event_bus
                    .send(ControllerMessage::P2P(ControllerP2PMessage::SparkTxs {
                        txs,
                        sender: addr,
                    }))
                    .await;
            }
            NetworkMessage::SparkSig(sigs) => {
                self.event_bus
                    .send(ControllerMessage::P2P(
                        ControllerP2PMessage::SparkSignatureData {
                            data: sigs,
                            sender: addr,
                        },
                    ))
            ...
            NetworkMessage::LtxidRelay => {
                self.peermgr.received_ltxidrelay(&addr);
            }
            NetworkMessage::Ltxidack => {
                self.peermgr.received_ltxidack(&addr, self.last_tick);
            }
