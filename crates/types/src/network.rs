...
impl Network {
    pub fn to_bitcoin_network(&self) -> BitcoinNetwork {
        match self {
            ...
            Network::Testnet | Network::Mutiny => BitcoinNetwork::Testnet,
        }
    }
    ...
    /// Returns the block that contains the very first LRC20 transaction for the given network.
    ...
    /// - `network::Testnet`
    /// - `network::Mutiny`
    pub fn lrc20_genesis_block(&self) -> Option<BlockHash> {
        let network_str = self.get_block_by_network()?;
    ...
    fn get_block_by_network(&self) -> Option<String> {
        match self {
            ...
            Network::Regtest => Some(REGTEST_GENESIS_BLOCK.into()),
