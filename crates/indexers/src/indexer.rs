...
use bitcoin_client::{BitcoinRpcApi, BitcoinRpcClient, json::GetBlockTxResult};
...
use lrc20_types::{ControllerMessage, IndexerMessage, network::Network};
use std::sync::Arc;
...
pub struct BitcoinBlockIndexer<BC, NS>
where
    BC: BitcoinRpcApi + Send + Sync + 'static,
    NS: PgDatabaseConnectionManager + IndexerNodeStorage + Send + Sync + 'static,
{
    /// Bitcoin RPC Client.
    ...
    /// Contains the time of the best indexed block.
    confirmed_index_block_time: Duration,
    /// Period of time to check Bitcoin node liveness. If it is set to None, Bitcoin node liveness
    /// is not checked.
...
impl<BC, NS> BitcoinBlockIndexer<BC, NS>
where
    NS: PgDatabaseConnectionManager + IndexerNodeStorage + Send + Sync + 'static,
    BC: BitcoinRpcApi + Send + Sync + 'static,
{
    pub fn new(
        ...
        Self {
            ...
            confirmed_index_block_time: Duration::from_secs(0),
    ...
    pub async fn init(
        ...
        let best_block_hash = self.bitcoin_client.get_best_block_hash().await?;
        let best_block = self.bitcoin_client.get_block(&best_block_hash).await?;

        if !self.is_bnode_synced(Duration::from_secs(best_block.header.time as u64)) {
            bail!("Bitcoin node is not synced since no new blocks were received in specified time");
        }
        ...
        let last_indexed_hash = match self.storage.get_last_indexed_block_hash().await? {
        ...
        self.confirmed_index_block_time = SystemTime::now().duration_since(UNIX_EPOCH)?;
        ...
        self.event_bus
            .send(ControllerMessage::InitialIndexingFinished)
            .await;

        Ok(())
    }

    /// Returns LRC20 genesis block height for the given network
    ...
    async fn get_starting_block_height(&self, params: &IndexingParams) -> eyre::Result<usize> {
        // Starting block height depends on the LRC20 genesis block for the given network.
        ...
        let mut starting_block_height =
            if let Some(starting_block_by_network) = self.network.lrc20_genesis_block() {
                self.bitcoin_client
        ...
        if let Some(last_indexed_hash) = self.storage.get_last_indexed_block_hash().await? {
        ...
        if let Some(staring_block_hash) = params.starting_block_hash {
            let params_block_height = self.get_block_height(&staring_block_hash).await?;
            let force_reindex = params.force_reindex.unwrap_or_default();

            if params_block_height > starting_block_height || force_reindex {
                starting_block_height = params_block_height;
            }
        }

        tracing::info!("Using starting block height {}", starting_block_height);
    ...
    pub async fn run(mut self, params: RunParams, cancellation: CancellationToken) {
        ...
        loop {
            ...
            let mut bnode_err = self.handle_new_blocks().await.err();

            if bnode_err.is_none() && !self.is_bnode_synced(self.confirmed_index_block_time) {
                bnode_err = Some(eyre!(
                    "Bitcoin node is not synced since no new blocks were received in the specified time."
                ))
            }

            if let Some(err) = bnode_err {
    ...
    /// Checks if the Bitcoin node is synced by computing a difference between best block and
    /// current time.
    fn is_bnode_synced(&self, confirmed_best_block_time: Duration) -> bool {
        self.liveness_period
            .map(|liveness_period| {
                let last_block_time = SystemTime::UNIX_EPOCH + confirmed_best_block_time;
    ...
    async fn index_block(&mut self, block: &GetBlockTxResult) -> eyre::Result<()> {
        ...
        self.storage
            .update_indexer_state(block.block_data.hash)
            .await?;
