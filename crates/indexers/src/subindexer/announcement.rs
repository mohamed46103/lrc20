...
use lrc20_types::announcements::{ParseOpReturnError, announcement_from_script};
use lrc20_types::{ControllerMessage, Lrc20Transaction, Lrc20TxType, network::Network};
...
impl AnnouncementsIndexer {
    ...
    async fn find_announcements(&self, block: &GetBlockTxResult) -> eyre::Result<()> {
        ...
        for tx in &block.tx {
            ...
            txs.push(Lrc20Transaction {
                bitcoin_tx: tx.clone(),
                tx_type: Lrc20TxType::Announcement(announcement),
            })
        }
