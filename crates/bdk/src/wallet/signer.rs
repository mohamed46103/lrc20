    ...
    fn sign_input(
        ...
        match self.ctx {
            SignerContext::Segwitv0 | SignerContext::Legacy => {
                ...
                let (msg, sighash_type) = match psbt.sighash_ecdsa(input_index, &mut sighasher) {
                    Ok(result) => result,
                    Err(SignError::WrongSigningAlgorithm) => {
                        return Ok(());
                    }
                    Err(e) => {
                        return Err(SignerError::Psbt(e));
                    }
                };
...
fn compute_tap_sighash(
    ...
    Ok((
        cache
            ...
            .map_err(|e| SignerError::SighashError(e))?,
        sighash_type,
    ))
}
