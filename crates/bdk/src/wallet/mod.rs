...
use bitcoin::{
    Address, Network, OutPoint, Script, ScriptBuf, Sequence, Transaction, TxOut, Txid, Weight,
    Witness, absolute,
};
...
use utils::{After, Older, SecpCtx, check_nsequence_rbf};
...
use crate::descriptor::{
    ...
    Policy, XKeyUtils, calc_checksum, into_wallet_descriptor_checked,
};
    ...
    D: BatchDatabase,
{
    ...
    pub fn get_balance(&self) -> Result<Balance, Error> {
        ...
        for u in utxos {
            ...
            if let Some(tx_conf_time) = &tx.confirmation_time {
                if tx.transaction.expect("No transaction").is_coinbase()
                    && (last_sync_height - tx_conf_time.height) < COINBASE_MATURITY
                {
                    immature += u.txout.value.to_sat();
                } else {
                    confirmed += u.txout.value.to_sat();
                }
            } else if u.keychain == KeychainKind::Internal {
                trusted_pending += u.txout.value.to_sat();
            } else {
                untrusted_pending += u.txout.value.to_sat();
            }
        }
    ...
    pub(crate) fn create_tx<Cs: coin_selection::CoinSelectionAlgorithm<D>>(
        ...
        coin_selection: Cs,
        params: TxParams,
    ) -> Result<(psbt::Psbt, TransactionDetails), Error> {
        ...
        let lock_time = match params.locktime {
            ...
            Some(x) => {
                return Err(Error::Generic(format!(
                    "TxBuilder requested timelock of `{:?}`, but at least `{:?}` is required to spend from this script",
                    x,
                    requirements.timelock.unwrap()
                )));
            }
        };
    ...
    #[allow(missing_docs)]
    pub fn sign(&self, psbt: &mut psbt::Psbt, sign_options: SignOptions) -> Result<bool, Error> {
    ...
    fn update_psbt_with_descriptor(&self, psbt: &mut psbt::Psbt) -> Result<(), Error> {
